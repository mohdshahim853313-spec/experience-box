...
import { Camera, User, Shield, Palette, Settings as SettingsIcon, LogOut, Moon, Sun, Monitor, Info, Mail, FileText, AlertCircle, Bug, Heart, ChevronRight, X, Upload, ArrowLeft, UserPlus, UserCheck, Users, Power } from "lucide-react";
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="glass-card p-6 rounded-2xl space-y-4 md:col-span-2">
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between p-4 rounded-xl border border-slate-200 bg-white/50">
                  <div className="flex items-center gap-4 mb-4 sm:mb-0">
                    <div className="bg-rose-100 p-2.5 rounded-xl">
                      <Mail className="w-6 h-6 text-rose-600" />
                    </div>
                    <div>
                      <h3 className="font-bold text-slate-800">Gmail Integration</h3>
                      <p className="text-sm text-slate-500">{needsAuth ? "Not connected" : "Connected & Syncing emails"}</p>
                    </div>
                  </div>
                  ...
                  {needsAuth ? (
                    <div className="flex flex-col items-end gap-2 w-full sm:w-auto">
                      {loginError && (
                        <p className="text-xs font-bold text-rose-500 max-w-[200px] text-right">{loginError}</p>
                      )}
                      <button 
                        onClick={handleGoogleLogin} 
                        disabled={isLoggingIn}
                        className="w-full sm:w-auto px-6 py-2.5 bg-white border border-slate-300 text-slate-700 font-bold rounded-xl hover:bg-slate-50 shadow-sm transition-all flex items-center justify-center gap-2 disabled:opacity-70"
                      >
                    ...
                    <button 
                      ...
                      className="w-full sm:w-auto px-6 py-2.5 bg-rose-50 text-rose-600 font-bold rounded-xl hover:bg-rose-100 transition-all"
                    >
                      Disconnect
                    </button>
                  )}
